...
from data import get_games, get_file_hash, sync_files
import sys
import logging

from PySide6.QtWidgets import QFileDialog, QMessageBox, QHBoxLayout, QListWidgetItem
...
class GameSyncApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.dropbox_label = QLabel("Dropbox not connected", alignment=Qt.AlignCenter)
        ...
        self.dropbox_folder_label = QLabel("Dropbox folder not found", alignment=Qt.AlignCenter)
    ...
    def init_ui(self):
        ...
        # Add a label
        self.label = QLabel("Click the button to retrieve games from the database.", alignment=Qt.AlignCenter)
        ...
        self.saves_label = QLabel("No Saves Folder Set", alignment=Qt.AlignCenter)
        if self.config_manager.config["local_saves_path"]:
            self.saves_label.setText(self.config_manager.config["local_saves_path"])
        ...
        self.dropbox_label = QLabel("Dropbox Status: Disconnected", alignment=Qt.AlignCenter)
        ...
        self.dropbox_folder_label = QLabel("No Dropbox Folder Selected", alignment=Qt.AlignCenter)
        ...
        self.sync_button.setEnabled(False)  # Disabled until configuration is complete
    ...
    def retrieve_db_info(self):
        ...
        try:
            ...
            games = get_games("none")  # Replace "none" with your actual database path
            if not games:
            ...
            for game in games:
                ...
                print(game)
            self.label.setText(f"Retrieved {len(games)} games from the database.")
    ...
    def select_delta_db(self):
        ...
        if file_path:
            self.config_manager.config["delta_db_path"] = file_path
            self.config_manager.save_config()
            self.db_label.setText(self.config_manager.config["delta_db_path"])
            self.delta_db_button.setText(f"Delta DB: {os.path.basename(file_path)}")
            self.update_sync_button()
    ...
    def select_local_saves(self):
        ...
        if folder_path:
            self.config_manager.config["local_saves_path"] = folder_path
            self.saves_label.setText(self.config_manager.config["local_saves_path"])
            self.config_manager.save_config()
            self.local_saves_button.setText(f"Saves: {os.path.basename(folder_path)}")
            self.update_sync_button()
    ...
    def connect_to_dropbox(self):
        ...
        if self.dropbox_manager.initialize_from_token():
            ...
            # # Try to auto-detect Delta folder
    ...
    def select_dropbox_folder(self):
        ...
        # This would ideally use a custom dialog to browse Dropbox folders
        # For simplicity, we'll just use a predefined path for now
        if not self.dropbox_manager.dbx:
            QMessageBox.warning(self, "Not Connected", "Please connect to Dropbox first")
            return
        folders = self.dropbox_manager.list_folders("")
        print(f"Folders in root: {folders}")
        if not folders:
            QMessageBox.warning(self, "Error", "Failed to list Dropbox folders")
            return
        
        # Simple implementation - in a real app, show a folder browser
        ...
        if ok and folder:
            ...
            self.config_manager.config["dropbox_folder_path"] = folder_path
            self.config_manager.save_config()
    ...
    def init_dropbox(self):
        ...
        if self.dropbox_manager.initialize_from_token():
            ...
            if self.config_manager.config["dropbox_folder_path"]:
                self.dropbox_folder_label.setText(self.config_manager.config["dropbox_folder_path"])
